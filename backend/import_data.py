...
def assign_funding_tier(row):
    ...
    if tuition <= 5 and country in [
        ...
        "Switzerland",
        "Israel",
    ] or (country == "USA" and tier in ["Tier 2", "Mid-Tier"]):
...
def import_masters_programs():
    ...
    logger.info("Importing %d programs from Excel...", len(df))
    ...
    for idx, row in df.iterrows():
        ...
        if uni_key not in universities:
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (row["University"], row["Country"], row["Region"], row["Tier"]),
            )
        ...
        """,
            (
                ...
                int(row["Net 10yr Cum ($K)"])
                if pd.notna(row["Net 10yr Cum ($K)"])
                else None,
                row["funding_tier"],
    ...
    logger.info("Import complete! %d universities, %d programs", uni_count, prog_count)
